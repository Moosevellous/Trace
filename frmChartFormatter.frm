...
Begin {C62A69F0-16DC-11CE-9E98-00AA00574A4F} frmChartFormatter 
   Caption         =   "Chart Formatter"
   ClientHeight    =   9285
   ClientLeft      =   120
   ClientTop       =   465
   ClientWidth     =   11205
   OleObjectBlob   =   "frmChartFormatter.frx":0000
...
Sub ApplyMarkerFill()
    ...
    For i = 1 To ActiveChart.FullSeriesCollection.Count
        With ActiveChart.FullSeriesCollection(i).Format.Fill
...
Sub ApplyMinorGridlines()
    If ActiveChart.Axes(xlValue).ScaleType <> xlScaleLogarithmic Then
        ...
        ActiveChart.Axes(xlValue).HasMinorGridlines = True
            ...
            Else
                If Me.txtMinorGridValue.Value <> "" Then ActiveChart.Axes(xlValue).MinorUnit = Me.txtMinorGridValue.Value
            End If
        Else
...
Sub ApplyMajorGridlines()
    ...
    ActiveChart.Axes(xlValue).HasMajorGridlines = True
        ...
        Else
            If IsNumeric(Me.txtMajorGridValue.Value) And Me.txtMajorGridValue.Value <> 0 Then ActiveChart.Axes(xlValue).MajorUnit = Me.txtMajorGridValue.Value
        End If
    Else
